...
<div class="content read">
	...
	<table class = "table">
        ...
        <tbody>
            ...
            <tr>
                <td><?=$language['linguaId']?></td>
                <td><?=$language['nome']?></td>
                <td><?=$language['percentagem']?></td>
                <td><?=$language['userId']?></td>
                <?php if ($_SESSION["idcargo"] == 1): ?>
                <td class="actions">
                    <a href="update.php?linguaId=<?=$language['linguaId']?>" class="update"><button class = "b">edit</button></a>
                    <a href="delete.php?linguaId=<?=$language['linguaId']?>" class="delete"><button class = "b">delete</button></a>
                </td>
                <?php endif; ?>
            </tr>
            <?php endforeach; ?>
